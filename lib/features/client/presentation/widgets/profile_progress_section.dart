  ...
  Widget build(BuildContext context) {
    return BlocListener<UserBloc, UserState>(
      listener: (context, userState) {
        if (userState is UserLoaded) _handleCompletion(userState.user.profileCompletion);
      },
      child: BlocListener<ProfileBloc, ProfileState>(
        ...
        child: BlocBuilder<ProfileBloc, ProfileState>(
          builder: (context, profileState) {
            int? completion;
            if (profileState is ProfileDocumentsLoaded) completion = profileState.completion.completion;
            else if (profileState is ProfileDocumentUploadedSuccess) completion = profileState.completion.completion;
            else if (profileState is ProfileDocumentUploadedNeedsVerification) completion = profileState.completion.completion;
            else if (profileState is ProfileDocumentPatched) completion = profileState.completion.completion;
            else if (profileState is ProfileDocumentDeleted) completion = profileState.completion.completion;
            else if (profileState is ProfileDocumentUploading) completion = profileState.completion.completion;

            if (completion == null) return const SizedBox.shrink();
            if (completion >= 100 && !_showCompletedBanner) return const SizedBox.shrink();
              ...
              return Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: AppColors.statusValideGreen.withValues(alpha: 0.3)),
                  ),
                  child: Row(
                    children: [
                      Icon(Icons.check_circle_outline, color: AppColors.statusValideGreen, size: 22),
                      const SizedBox(width: 10),
                      Text('home.profile_completed'.tr(),
                          style: TextStyle(fontFamily: AppConstants.fontFamilySofiaSans, fontWeight: FontWeight.w600, fontSize: 14, color: AppColors.statusValideGreen)),
                    ],
                  ),
                ),
              );
              ...
              child: Column(
                ...
                children: [
                  Text('home.complete_profile'.tr(),
                      style: TextStyle(fontFamily: AppConstants.fontFamilySofiaSans, fontWeight: FontWeight.w600, fontSize: 14, color: AppColors.primaryDark)),
                  const SizedBox(height: 10),
                  Stack(
                    alignment: Alignment.centerLeft,
                    children: [
                      Container(height: 7, decoration: BoxDecoration(color: AppColors.progressTrack, borderRadius: BorderRadius.circular(4))),
                      FractionallySizedBox(
                        widthFactor: progress,
                        child: Container(height: 7, decoration: BoxDecoration(color: AppColors.progressFill, borderRadius: BorderRadius.circular(4))),
                      ),
                    ],
                  ),
                  ...
                  Align(
                    ...
                    child: Text('$completion%',
                        style: TextStyle(fontFamily: AppConstants.fontFamilyInter, fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.progressFill)),
                  ),
                  const SizedBox(height: 10),
                  GestureDetector(
                    onTap: () => Navigator.of(context).push(MaterialPageRoute(
                      builder: (_) => BlocProvider.value(value: context.read<ProfileBloc>(), child: const Step2DocumentsScreen()),
                    )),
                    child: Container(
                      height: 46,
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text('home.start_now'.tr(),
                              style: TextStyle(fontFamily: AppConstants.fontFamilySofiaSans, color: AppColors.white, fontWeight: FontWeight.w600, fontSize: 14)),
                          const SizedBox(width: 12),
                          Container(
                            width: 30, height: 30,
                            decoration: const BoxDecoration(color: AppColors.white, shape: BoxShape.circle),
                            child: Icon(Icons.arrow_forward, color: AppColors.primaryDarker, size: 16),
                          ),
