          ...
          children: [
            ...
            Expanded(
              child: TextField(
                ...
                onSubmitted: _search,
                style: TextStyle(fontFamily: AppConstants.fontFamilyInter, fontSize: AppConstants.fontSizeMedium, color: AppColors.textBlack87),
                decoration: InputDecoration(
                  hintText: 'home.search_placeholder'.tr(),
                  hintStyle: TextStyle(fontFamily: AppConstants.fontFamilyInter, fontSize: AppConstants.fontSizeMedium, color: AppColors.greyShade500, fontWeight: FontWeight.w400),
            ...
            GestureDetector(
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  child: Icon(Icons.arrow_forward, color: AppColors.white, size: 18),
                ),
              ),
            ),
          ],
          ...
          Row(
            ...
            children: [
              Text('home.recent_requests'.tr(),
                  style: TextStyle(fontFamily: AppConstants.fontFamilySofiaSans, fontWeight: FontWeight.w500, fontSize: AppConstants.fontSizeTitle, color: AppColors.textBlack87)),
              GestureDetector(
                onTap: () => Navigator.of(context).pushNamed(AppRoutes.clientDemandes),
                child: Row(
                  children: [
                    Text('home.see_all'.tr(),
                        style: TextStyle(fontFamily: AppConstants.fontFamilyInter, fontWeight: FontWeight.bold, fontSize: AppConstants.fontSizeMedium, color: AppColors.primary)),
                    const SizedBox(width: 4),
                    Icon(Icons.arrow_forward_ios, size: AppConstants.fontSizeSmall, color: AppColors.primary),
                  ],
                ),
              ),
            ],
          ...
          BlocBuilder<DemandesBloc, DemandesState>(
            builder: (context, state) {
              if (state is DemandesLoading) {
                return const Center(child: Padding(padding: EdgeInsets.symmetric(vertical: 24), child: CircularProgressIndicator(strokeWidth: 2)));
              }
              if (state is DemandesError) {
                return Padding(
                  padding: const EdgeInsets.symmetric(vertical: 16),
                  child: Text(state.message, style: TextStyle(fontFamily: AppConstants.fontFamilyInter, fontSize: AppConstants.fontSizeRegular, color: AppColors.statusRejected)),
                );
              }
              if (state is RecentDemandesLoaded) {
                if (state.demandes.isEmpty) {
                  return Padding(
                    padding: const EdgeInsets.symmetric(vertical: 16),
                    child: Text('demandes.no_demandes'.tr(), style: TextStyle(fontFamily: AppConstants.fontFamilyInter, fontSize: AppConstants.fontSizeRegular, color: AppColors.textSecondary)),
                  );
                }
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) => RecentDemandeCard(demande: state.demandes[index]),
                );
              }
...
class RecentDemandeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final title = demande.formType.isNotEmpty ? demande.formType : demande.requestNumber;
    ...
    return GestureDetector(
      onTap: () => Navigator.of(context).pushNamed(AppRoutes.clientDemandeDetail, arguments: {'id': demande.id}),
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [BoxShadow(color: AppColors.shadowDark, blurRadius: 6, offset: Offset(0, 2))],
        ),
        child: Row(
          children: [
            Container(
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark), overflow: TextOverflow.ellipsis),
            ...
            Container(
              ...
              decoration: BoxDecoration(color: statusCfg.bgColor, borderRadius: BorderRadius.circular(20)),
              child: Text(statusCfg.label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: statusCfg.textColor)),
            ),
          ],
        ),
      ),
