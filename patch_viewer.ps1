$f = 'lib\features\client\presentation\pages\mes_documents_screen.dart'
$lines = [System.IO.File]::ReadAllLines($f)

# Trouver les bornes de _DocumentViewerSheet et _PdfBytesViewer
$startViewer = -1
$startPdf = -1
for ($i = 0; $i -lt $lines.Length; $i++) {
    if ($lines[$i] -match '^class _DocumentViewerSheet ' -and $startViewer -eq -1) { $startViewer = $i }
    if ($lines[$i] -match '^class _PdfBytesViewer ' -and $startPdf -eq -1) { $startPdf = $i }
}

Write-Host "startViewer=$startViewer startPdf=$startPdf"

$before = $lines[0..($startViewer-1)]
$after  = $lines[$startPdf..($lines.Length-1)]
...
Write-Host "Patch done. Total lines: $($newLines.Length)"
